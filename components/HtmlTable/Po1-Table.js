...
export default class PoTable1 extends Component{

  constructor(props){
    super(props)
    this.checkObjeByBaseInfo=this.checkObjeByBaseInfo.bind(this)
    this.checkArryByGoodsInfo=this.checkArryByGoodsInfo.bind(this)
  }
...
//判断下标，因为title 数量有限，当合并列的时候，需要把除了第一条商品，其他的不进行渲染，所以只把下标为0的第一条商品信息，的title进行赋满值，其他的少赋值
              ...
              data.rowSpan ? (
                  <td rowSpan={datagoodslength}>
                    <div  className="bodyBox">
                      {
                      data.render ? data.render(data,dataInfo,goodsData):<span>{data.value}</span>
                      }
                    </div>
                  </td>
              ):(
    ...
    baseInfoList1(baseInfo,baseInfoTitle,index){
      var list=[]
      debugger;
